...
describe('Avoids front-end JavaScript libraries with known vulnerabilities', () => {
  ...
  it('fails when JS libraries with known vulnerabilities are detected', () => {
    ...
    // TODO: investigate if this is properly handled by renderer
    assert.equal(auditResult.details.items[0].detectedLib.label, 'angular@1.1.4');
    assert.equal(auditResult.details.items[0].detectedLib.value, 'https://snyk.io/vuln/npm:angular?lh@1.1.4');
  });
  ...
  it('handles ill-specified versions', () => {
    ...
    assert.equal(auditResult.rawValue, false);
    assert.equal(auditResult.details.items.length, 1);
    assert.equal(auditResult.details.items[0].detectedLib.label, 'jquery@1.8.0');
  });
